...
import { UserProfile, CropConfig, GlobalSystemData, RaidOrder, GuildComment } from '../types.ts';
...
export default function Guild({ language = 'en', profile, globalData, messages: initialMessages, isAdmin, onDeleteMessage, onUpdateBossHp, onSendMessage }: GuildProps) {
  const [messages, setMessages] = useState<Message[]>(initialMessages);
  const [activeTab, setActiveTab] = useState<'orders' | 'pack' | 'allies' | 'messages'>('pack');
  ...
  const toggleLike = (msgId: string) => {
    setMessages(prev => prev.map(msg => {
        if (msg.id === msgId) {
            ...
            return {
                ...msg,
                supports: isLiked ? msg.supports - 1 : msg.supports + 1,
                userReaction: isLiked ? undefined : 'like'
            };
        }
        return msg;
    }));
  ...
  const handleChatUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          reader.onload = (ev) => {
              if (onSendMessage) onSendMessage(composerText, ev.target?.result as string);
              setComposerText('');
          };
  ...
  const handleRaidUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          const reader = new FileReader();
          reader.onload = (ev) => {
              setTempCropImg(ev.target?.result as string);
              setIsCropping(true);
              setCropScale(1);
              setCropPos({ x: 0, y: 0 });
          };
  ...
  const finalizeCrop = () => {
      if (tempCropImg && canvasRef.current && viewportRef.current) {
          ...
          img.onload = () => {
              ...
              // viewportH is calculated by CSS aspect ratio, but we know it's 9:16
  ...
  const submitRaid = () => {
      ...
      // Send to chat as proof
      if (onSendMessage && selectedOrder) {
          const msg = `[RAID EXECUTED: ${selectedOrder.title}]\n${proofText}`;
          onSendMessage(msg, proofMedia || undefined);
      }
    ...
    <div className="max-w-2xl mx-auto flex flex-col min-h-screen bg-black" onMouseUp={() => setIsDragging(false)} onTouchEnd={() => setIsDragging(false)}>
      ...
      <section className="p-4 border-b border-[#00FF88]/20 bg-black sticky top-0 z-50 relative">
        <div className="flex justify-between items-end mb-2">
          <div><h2 className="text-[9px] uppercase tracking-widest text-gray-500 font-bold">Raid Operations</h2><h1 className="text-2xl font-black uppercase italic text-white tracking-tighter leading-none mb-1">{globalData?.guildBoss.name || 'VOID TITAN'}</h1></div>
          <span className="text-[10px] data-font text-white font-bold">{bossHp.toLocaleString()} HP</span>
        </div>
        <div className="h-1.5 bg-[#111] mb-4"><div className="h-full toxic-bg shadow-[0_0_10px_#00FF88]" style={{ width: `${(bossHp / (globalData?.guildBoss.maxHp || 1000000)) * 100}%` }}></div></div>
        
        {damageFeedback && (
            <div className="absolute bottom-[-20px] right-4 text-[#00FF88] font-black text-xl damage-float z-50">
                {damageFeedback}
            </div>
        )}
      </section>
      ...
      <nav className="flex border-b border-[#111] h-16 bg-black/95">
        <button onClick={() => setActiveTab('orders')} className={`flex-1 flex flex-col items-center justify-center ${activeTab === 'orders' ? 'active-tab toxic-text' : 'text-gray-500'}`}><span className="text-lg">💀</span><span className="text-[8px] uppercase font-black tracking-widest mt-1">Raid Orders</span></button>
        <button onClick={() => setActiveTab('pack')} className={`flex-1 flex flex-col items-center justify-center ${activeTab === 'pack' ? 'active-tab toxic-text' : 'text-gray-500'}`}><span className="text-lg">🐺</span><span className="text-[8px] uppercase font-black tracking-widest mt-1">The Pack</span></button>
        <button onClick={() => setActiveTab('allies')} className={`flex-1 flex flex-col items-center justify-center ${activeTab === 'allies' ? 'active-tab toxic-text' : 'text-gray-500'}`}><span className="text-lg">👊</span><span className="text-[8px] uppercase font-black tracking-widest mt-1">Allies</span></button>
      </nav>

      <main className={`flex-grow p-4 overflow-y-auto no-scrollbar ${activeTab === 'pack' ? 'pb-56' : 'pb-24'}`}>
        {activeTab === 'orders' && (
        ...
        {activeTab === 'allies' && (
          <div className="space-y-4 animate-fade-in">
            <div className="border border-[#00FF88]/20 bg-[#00FF88]/5 p-4">
              <h3 className="text-xs font-black text-[#00FF88] uppercase tracking-widest mb-1">🏆 Raid Boss Leaderboard</h3>
              <p className="text-[9px] text-gray-500 mb-4 uppercase">Versla de baas. Win prijzen. Eeuwige roem.</p>

              <div className="space-y-2 mb-6">
                {/* Dummy leaderboard — in productie gevuld vanuit Firestore aggregation */}
                {[
                  { rank: 1, name: 'SHADOW_WOLF', guild: 'Iron Pack', dmg: 47500, prize: '🎁 GRATIS' },
                  { rank: 2, name: 'FENRIR_X',    guild: 'Iron Pack', dmg: 38200, prize: '50% KORTING' },
                  { rank: 3, name: 'VOID_HUNTER', guild: 'Ghost Den', dmg: 31100, prize: '—' },
                ].map(p => (
                  <div key={p.rank} className="flex items-center justify-between border border-[#222] p-3">
                    <div className="flex items-center gap-3">
                      <span className={`text-sm font-black w-6 text-center ${p.rank === 1 ? 'text-[#FFB800]' : p.rank === 2 ? 'text-gray-400' : 'text-[#CD7F32]'}`}>
                        {p.rank === 1 ? '🥇' : p.rank === 2 ? '🥈' : '🥉'}
                      </span>
                      <div>
                        <p className="text-[10px] font-black text-white uppercase">{p.name}</p>
                        <p className="text-[8px] text-gray-600 uppercase">{p.guild}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] data-font text-[#00FF88] font-black">{p.dmg.toLocaleString()} DMG</p>
                      {p.prize !== '—' && (
                        <p className="text-[8px] font-black text-[#FFB800] uppercase">{p.prize}</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="border-t border-[#111] pt-4">
                <h4 className="text-[9px] font-black text-[#00FF88] uppercase tracking-widest mb-2">🐺 Guild Rankings</h4>
                <div className="space-y-2">
                  {[
                    { rank: 1, name: 'Iron Pack',  points: 142500, badge: '⚡ APEX HUNTERS' },
                    { rank: 2, name: 'Ghost Den',   points: 98300,  badge: '🌑 SHADOW RUNNERS' },
                    { rank: 3, name: 'Steel Fang',  points: 71200,  badge: '🔱 IRON WOLVES' },
                  ].map(g => (
                    <div key={g.rank} className="flex items-center justify-between border border-[#1a1a1a] p-3">
                      <div>
                        <p className="text-[10px] font-black text-white uppercase">
                          {g.name}
                          <span className="ml-2 text-[8px] text-[#00FF88] font-bold">{g.badge}</span>
                        </p>
                      </div>
                      <p className="text-[10px] data-font text-[#00FF88]">{g.points.toLocaleString()} pts</p>
                    </div>
                  ))}
                </div>
              </div>

              <div className="mt-4 border border-[#FFB800]/30 bg-[#FFB800]/5 p-3">
                <p className="text-[9px] font-black text-[#FFB800] uppercase tracking-widest mb-1">🎯 Beloningen deze maand</p>
                <ul className="text-[9px] text-gray-400 space-y-1">
                  <li>🥇 #1 Speler → <span className="text-[#00FF88]">Gratis prijs uit de Armory</span></li>
                  <li>🥈 #2 Speler → <span className="text-[#00FF88]">50% korting op elke aankoop</span></li>
                  <li>🏆 Top 3 Guild (alle leden) → <span className="text-[#00FF88]">10% korting</span></li>
                  <li>🐺 Top 3 Guilds → <span className="text-[#00FF88]">Eeuwige bijnaam naast guildnaam</span></li>
                </ul>
              </div>
            </div>
          </div>
              ...
              <div key={msg.id} className="bg-[#0a0a0a] border border-[#111] p-5 relative group">
                {isAdmin && onDeleteMessage && (
                  <button 
                    ...
                    className="absolute top-2 right-2 bg-red-900/20 text-red-500 border border-red-900/50 text-[8px] font-black uppercase px-2 py-1 hover:bg-red-600 hover:text-white transition-all opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
                  >
                    Delete
                  </button>
                )}
                <div className="flex gap-4 mb-4">
                  <div className="w-10 h-10 border border-[#00FF88]/30 overflow-hidden bg-black flex items-center justify-center">{msg.userPic ? <img src={msg.userPic} className="w-full h-full object-cover" /> : <span className="text-[10px] toxic-text font-black">{msg.rank}</span>}</div>
                ...
                <div className="flex gap-6 pt-4 border-t border-[#111] items-center">
                   <button 
                     ...
                     className={`text-[10px] font-black uppercase tracking-widest hover:text-[#00FF88] transition-colors ${msg.userReaction ? 'text-[#00FF88]' : 'text-gray-600'}`}
                   >
                     👊 {msg.supports}
                   </button>
                   <button 
                     onClick={() => setActiveCommentId(activeCommentId === msg.id ? null : msg.id)}
                     className="text-[10px] font-black text-gray-600 uppercase tracking-widest hover:text-[#00FF88]"
                   >
                    ...
                    <div className="mt-4 bg-[#050505] p-3 border-t border-[#111]">
                        ...
                        <div className="flex gap-2">
                            <input 
                                ...
                                className="flex-grow bg-black border border-[#222] p-2 text-[10px] text-white outline-none focus:border-[#00FF88]"
                                onKeyDown={e => e.key === 'Enter' && addComment(msg.id)}
                            />
      ...
      {activeTab === 'pack' && (
        <div className="fixed bottom-20 left-0 w-full bg-[#050505]/95 backdrop-blur-md border-t border-[#00FF88]/20 p-4 z-40">
           <div className="max-w-2xl mx-auto flex items-center gap-3">
              ...
              <input value={composerText} onChange={e => setComposerText(e.target.value)} placeholder="Transmit intelligence..." className="flex-grow bg-black border border-[#222] p-3 text-[11px] font-black text-white outline-none focus:border-[#00FF88]" />
      ...
      {selectedOrder && !isCropping && (
          <div className="fixed inset-0 z-[1001] bg-black/95 flex items-center justify-center p-4 overflow-y-auto">
              <div className="w-full max-w-sm bg-[#0a0a0a] border border-[#00FF88] p-6 shadow-[0_0_30px_#00FF8830] animate-slide-in my-auto">
                  ...
                  <div 
                    onClick={() => raidFileRef.current?.click()}
                    className="h-[150px] aspect-[9/16] bg-black border border-dashed border-[#333] mb-4 flex items-center justify-center cursor-pointer hover:border-[#00FF88] transition-all relative overflow-hidden group mx-auto"
                  >
                      {proofMedia ? (
                          <img src={proofMedia} className="w-full h-full object-cover" />
                      ) : (
                          <div className="text-center">
                              <div className="text-3xl mb-2">📷</div>
                              <span className="text-[9px] uppercase font-black text-gray-500">Tap to Upload (9:16)</span>
                          </div>
                      )}
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                          <span className="text-xs font-black text-[#00FF88] uppercase">Change Image</span>
                      </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                      <button onClick={() => setSelectedOrder(null)} className="flex-1 py-3 border border-gray-800 text-gray-500 font-black uppercase text-[10px]">Cancel</button>
      ...
      {isCropping && tempCropImg && (
          <div className="fixed inset-0 z-[150] bg-black flex flex-col items-center justify-center p-4">
              ...
              <div className="w-full max-w-sm space-y-4">
                  ...
                  <div 
                    ref={viewportRef}
                    className="relative w-full h-[300px] border-2 border-[#00FF88] overflow-hidden bg-[#111]"
                    onMouseDown={onPanStart} onMouseMove={onPanMove} onTouchStart={onPanStart} onTouchMove={onPanMove}
                  >
                      ...
                      <div 
                        ...
                        style={{ transform: `translate(${cropPos.x}px, ${cropPos.y}px) scale(${cropScale})` }}
                      >
                  ...
                  <div className="bg-[#111] p-4 border border-[#222]">
                      ...
                      <div className="flex gap-4">
                          <button onClick={() => { setIsCropping(false); setTempCropImg(null); }} className="flex-1 py-3 border border-gray-700 text-gray-500 font-black uppercase text-[10px]">Cancel</button>
